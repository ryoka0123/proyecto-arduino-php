...
<body class="bg-[#101a23]">
    <div class="relative flex min-h-screen w-full flex-col"
        ...
        {{-- ========== HEADER ========== --}}
        <header
            class="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#223749] px-4 sm:px-10 py-3 text-white">
            ...
            <div class="flex flex-1 justify-end items-center gap-4 sm:gap-8">
                <div class="text-sm text-[#90b0cb] hidden md:block">Bienvenido, <span
                        class="font-bold text-white">{{ $username }}</span>!</div>
                <form method="post" action="{{ route('cerrarSesion') }}">
                    @csrf
                    <button type="submit"
                        class="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#223749] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#314d64] transition-colors">
        ...
        <main class="flex flex-1 justify-center p-4 sm:p-5 lg:px-40">
            <div class="w-full max-w-6xl">
                <div class="flex flex-wrap justify-between items-center gap-4 p-4">
                    ...
                    <a href="{{ route('registroArduino') }}"
                        class="flex cursor-pointer items-center justify-center rounded-lg h-9 px-5 bg-[#2094f3] text-white text-sm font-bold leading-normal hover:bg-[#1a7ad1] transition-colors">
                    ...
                    <div class="grid grid-cols-4 gap-5 p-4">
                        @foreach($arduinos as $arduino)
                            <div class="flex flex-col gap-3 rounded-lg bg-[#18232e] p-4 transition-transform hover:scale-105">
                                ...
                                <img src="{{ asset('image/ESP32.png') }}"
                                    alt="Imagen de un dispositivo ESP32" class="w-full aspect-video object-cover rounded-md">
                                ...
                                <div class="mt-2 flex items-center gap-2">
                                    <a href="{{ route('triggers', $arduino->id) }}"
                                        class="flex-1 text-center bg-[#2094f3] text-white rounded-md py-2 text-sm font-bold hover:bg-[#1a7ad1] transition-colors">Ir</a>
                                    <button type="button"
                                        onclick="openEditArduinoModal({{ $arduino->id }}, '{{ addslashes($arduino->nombre) }}', '{{ addslashes($arduino->ip) }}')"
                                        class="flex-1 bg-[#4a5a6a] text-white rounded-md py-2 text-sm font-bold hover:bg-[#5f748a] transition-colors">Editar</button>
                                    <button type="button"
                                        onclick="openModal({{ $arduino->id }}, '{{ addslashes($arduino->nombre) }}')"
                                        class="flex-1 bg-[#e53e3e] text-white rounded-md py-2 text-sm font-bold hover:bg-[#c53030] transition-colors">Eliminar</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <form id="formEliminar" method="post">
                ...
                <div class="flex gap-4 mt-6">
                    <button type="button" onclick="closeModal()"
                        class="w-full py-2 bg-[#4a5a6a] text-white rounded-lg font-bold hover:bg-[#5f748a] transition-colors">Cancelar</button>
                    <button type="submit"
    ...
    <div id="editArduinoModal"
        ...
        <div class="bg-[#18232e] p-6 rounded-lg shadow-xl w-full max-w-sm border border-[#223749]">
            ...
            <form id="editArduinoForm" method="post">
                @csrf
                <div class="space-y-4">
                    <input type="text" id="editArduinoNombre" name="nombre" placeholder="Nombre del dispositivo"
                        required
                        class="w-full p-3 bg-[#101a23] text-white rounded-lg border border-[#22374a] focus:ring-2 focus:ring-[#2094f3] focus:outline-none">
                    <input type="text" id="editArduinoIp" name="ip" placeholder="Dirección IP" required
                        class="w-full p-3 bg-[#101a23] text-white rounded-lg border border-[#22374a] focus:ring-2 focus:ring-[#2094f3] focus:outline-none">
                </div>
                <div class="flex gap-4 mt-6">
                    <button type="button" onclick="closeEditArduinoModal()"
                        class="w-full py-2 bg-[#4a5a6a] text-white rounded-lg font-bold hover:bg-[#5f748a] transition-colors">Cancelar</button>
                    <button type="submit"
